...
class ParseBED(object):
    ...
    def __init__(self, gene_models):
        ...
        self.file_handler = open(gene_models,'r')
        print(self.file_handler)
        self.file_handlerileName=os.path.basename(gene_models)
        self.ABS_fileName=gene_models
    ...
    def complementBED(self,sizeFile,outfile=None):
        ...
        if outfile is None:
            outfile=self.file_handlerileName + ".compl.bed"
        FO=open(outfile,'w')
    ...
    def bedToWig(self,outfile=None,log2scale=False,header=True):
        ...
        if outfile is None:
            outfile = self.file_handlerileName + ".wig"
        FO=open(outfile,'w')
    ...
    def bedToGFF(self,outfile=None):
        ...
        if outfile is None:
            output_name = self.file_handlerileName + ".GFF"
        ...
        i = 0
        for i, line in enumerate( self.file_handler ):
            complete_bed = False
            line = line.rstrip( '\r\n' )
    ...
    def getBedinfor(self,outfile=None, reffa=None):
        ...
        if outfile is None:outfile = self.file_handlerileName + ".infor.xls"
        FO=open(outfile,'w')
    ...
    def filterBedbyIntronSize(self,outfile=None,min_intron=50):
        ...
        '''
        if outfile is None:outfile = self.file_handlerileName + ".filterIntron.xls"
        FO=open(outfile,'w')
        print("writing feature information to " + outfile + " ...", file=sys.stderr)             
    ...
    def getAllUniqJunctions(self,outfile=None,flankSize=20):
        ...
        file'''
        if outfile is None:outfile = self.file_handlerileName + ".uniqJunctions.bed"
        FO=open(outfile,'w')
    ...
    def collapseJunctionBed(self,outfile=None):
        ...
        if outfile is None:outfile = self.file_handlerileName + ".collapsed.bed"
        FO=open(outfile,'w')
        print("\tCollapse junctions for " +    self.file_handlerileName, end=' ', file=sys.stderr)  
    ...
    def filterJunctionBed(self,outfile=None,overhang=8,min_intron=50,max_intron=1000000,cvg=1):
        ...
        if outfile is None:outfile = self.file_handlerileName + ".filter.bed"
        FO=open(outfile,'w')
    ...
    def unionBED(self, outfile=None, outNameFile=None, boundary="utr", stranded=True):
        ...
        if outNameFile is None: outNameFile = self.file_handlerileName + ".name.txt"       
        FO=open(outfile,'w')
        ...
        FO.write("track name=" + self.file_handlerileName + " description=" + ' \"' + "Overlapping entries in " + self.file_handlerileName + " were merged" + '\"'+'\n') 
    ...
    def nrBED(self,outfile=None):
        ...
        if outfile is None:
            outfile = self.file_handlerileName + ".nr.bed"
        FO=open(outfile,'w')
        
        mergeGene=collections.defaultdict(list)
        print("Removing redundcany from " + self.file_handlerileName + "  ...", file=sys.stderr)
        for line in self.file_handler:
            if line.startswith('#'):continue
            if line.startswith('track'):continue
            if line.startswith('browser'):continue  
...
class CompareBED:
    ...
    def __init__(self,gene_modelsA,gene_modelsB):
        ...
        self.A_fh=open(gene_modelsA,'r')
        self.A_full_Name=gene_modelsA
        self.A_base_Name=os.path.basename(gene_modelsA)
    ...
    def annotateEvents(self,outfile=None):
        ...
        logger.info("This is test one %s ..." % self.A_full_name)
